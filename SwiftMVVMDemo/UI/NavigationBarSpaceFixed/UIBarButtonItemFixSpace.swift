...
extension UIImagePickerController {
    ...
    static let cm_swizzleMethod: Void = {
        DispatchQueue.once(UUID().uuidString, block: {
            swizzleMethod(UIImagePickerController.self, originalSelector: #selector(UIImagePickerController.viewWillAppear(_:)), swizzleSelector: #selector(UIImagePickerController.cm_viewWillAppear(_:)))
            
            swizzleMethod(UIImagePickerController.self, originalSelector: #selector(UIImagePickerController.viewWillDisappear(_:)), swizzleSelector: #selector(UIImagePickerController.cm_viewWillDisappear(_:)))
        })
    }()
    ...
    var tempDisableFixSpace: Bool {
        ...
        set {
            objc_setAssociatedObject(self, &AssociatedKeys.tempDisableFixSpace, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
...
extension UINavigationBar {
    ...
    static let cm_swizzleMethod: Void = {
        DispatchQueue.once(UUID().uuidString, block: {
            swizzleMethod(UINavigationBar.self, originalSelector: #selector(UINavigationBar.layoutSubviews), swizzleSelector: #selector(UINavigationBar.cm_layoutSubviews))
        })
    }()
...
extension UINavigationItem {
    ...
    open override func setValue(_ value: Any?, forKey key: String) {
        ...
        if #available(iOS 11.0, *) {
            ...
            if cm_disableFixSpace == false &&
                (key == BarButtonItem.left.rawValue || key == BarButtonItem.right.rawValue) {
